CREATE OR REPLACE PROCEDURE TPCDI_STG.PUBLIC.LOAD_HOLDING_HISTORY_I_SP(scale float,batches float,wait float)
	...
	while (batch_counter <= BATCHES)	
	{
		var incrm_stmt = snowflake.createStatement(
			{sqlText: "COPY INTO TPCDI_STG.PUBLIC.HOLDINGHISTORY_STG FROM @TPCDI_FILES/tpcdi-" + tpcdi_scale + "/Batch" + batch_counter + "/HoldingHistory FILE_FORMAT = (FORMAT_NAME = 'TXT_PIPE')"}
			);
		incrm_stmt.execute();
		// insert wait here
