...
# MAGIC %md
# MAGIC # TPCx-AI Use Case 09
# MAGIC Customer facial recognition
# MAGIC 
# MAGIC **Setup**:
...
# MAGIC %pip install tensorflow-addons opencv-python dlib
...
def align(data, width, height, origin):
    ...
    res_path = "/dbfs/user/mgroh@hitachisolutions.com/tpcx-ai-v1.0.2/workload/spark/pyspark/workload-pyspark/resources/uc09/shape_predictor_5_face_landmarks.dat"
...
def load_data(session: SparkSession, num_proc, base_path, path, stage) -> DataFrame:
    # meta_data = session.read.csv(path, sep=INPUT_SEPARATOR, inferSchema=True, header=True)
    meta_data = spark.read.format("delta").load(f"{path}/{stage}/CUSTOMER_IMAGES_META")
    ...
    if "identity" in data.columns:
        ...
        return data.select("sample", "image").cache()
...
images = load_data(spark, num_processes, base_path, data_path, "training")
...
# weights_path = res_path / 'nn4.small2.v1.h5'
weights_path = "/dbfs/user/mgroh@hitachisolutions.com/tpcx-ai-v1.0.2/workload/spark/pyspark/workload-pyspark/resources/uc09/nn4.small2.v1.h5"
...
# DBTITLE 1,Serving
images = load_data(spark, num_processes, base_path, data_path, "serving")
preprocessed_data = pre_process(images)
